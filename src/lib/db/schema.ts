...
export const ballRaces = pgTable("ballrace_races", {
  id: text("id").primaryKey(),
  size: integer("size").notNull(),
  status: text("status").notNull().default("waiting"),
  result: jsonb("result"),
  obstaclesSeed: text("obstacles_seed"),
  paymentId: text("payment_id"),
  paymentAlias: text("payment_alias"),
  paymentCvu: text("payment_cvu"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
...
export const ballSlots = pgTable("ballrace_slots", {
  id: serial("id").primaryKey(),
  raceId: text("race_id")
    .notNull()
    .references(() => ballRaces.id),
  ballIndex: integer("ball_index").notNull(),
  displayName: text("display_name").notNull(),
  xHandle: text("x_handle"),
  avatarUrl: text("avatar_url"),
  cuit: text("cuit"),
...
export const ballLeaderboard = pgTable("ballrace_leaderboard", {
  id: serial("id").primaryKey(),
  cuit: text("cuit").notNull().unique(),
  displayName: text("display_name").notNull(),
  avatarUrl: text("avatar_url"),
